...
def text2coco(text,df_full,cocolst):
    result = []
    for texti in text:
        t = df_full[df_full["anno_whole"]==texti].index.tolist()
        idx = t[0]
        result.append(cocolst[idx][0])
    return result
...
def find_embed(img_coco,mode='df'):
    ...
    for i in range(len(img_coco)):
        idx.append(cocofeat['files'].index(img_coco[i]))
    idx = np.array(idx)
    coco_embed = cocofeat['clipfeat'][idx]
...
if __name__ == "__main__":
    ...
    coco_dir = "variables/tangram_things_match_full.json"
    with open(coco_dir,"r") as f:
        coco = json.load(f)

    cocofeat = torch.load("variables/things_imgfeat.pt")
    ...
    coco_train = text2coco(train_wholetext,df_tangram_full,coco['tang2img'])
    coco_test = text2coco(test_wholetext,df_tangram_full,coco['tang2img'])
    ...
    for i in range(df_full.shape[0]):
        ...
        if tangname not in (train_img+test_img):
            ...
            imgcoco = text2coco([texti],df_tangram_full,coco['tang2img'])[0]
    ...
    # 4. 训练循环
    results = []
    save_dir = f"/home/nncc/lyx/project/EEG_Image_decode-main/Generation/fintune_ckpts/CLIP/sub-01-6161"
    for epoch in range(40):
        ...
        if (epoch+1)%1 == 0:
            ...
            # test eval
            dl = data2loader(test_dataset.img+train_dataset.img, coco_test+coco_train, train_transform, batch_size=32)
